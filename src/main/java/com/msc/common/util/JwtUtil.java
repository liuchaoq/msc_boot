...
import com.msc.common.vo.LoginUser;
import com.msc.common.vo.SysUserCacheInfo;
...
public class JwtUtil {
    ...
    public static final long EXPIRE_TIME_PC = 360 * 60 * 1000;

    public static final long EXPIRE_TIME_DINGDING = 1440 * 60 * 1000;
     ...
     * @return 加密的token
     */
    public static String sign(String username, String secret, String sysType, String loginType) {
        long time;
        if ("pc".equals(loginType)) {
            time = JwtUtil.EXPIRE_TIME_PC;
        } else {
            time = JwtUtil.EXPIRE_TIME_DINGDING;
        }
        Date date = new Date(System.currentTimeMillis() + time);
        Algorithm algorithm = Algorithm.HMAC256(secret);
        // 附带username信息
        return JWT.create().withClaim("username", username).withClaim("sysType", sysType).withClaim("loginType", loginType).withExpiresAt(date).sign(algorithm);
